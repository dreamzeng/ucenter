(function (window) {
    ...
    var AntHttpClient = function (option, mqttClientProxy) {
        ...
        this.createAjaxFun = function (params, callback, interface, isPost, error, dns, prePath) {
            ...
            // 追加delete, put方式，第三方可能有调用此方法
            var method = isPost ? "POST" : "GET";
            isPost == 'delete' ? method = 'DELETE' : '';
            isPost == 'put' ? method = 'PUT' : '';
            isPost == 'put' ? header = this.postHeader() : '';
            isPost == 'delete' ? header = this.getHeader() : '';
        ...
        this.RoomBusy = {
            ...
            // 添加会话
            addChatList: function (params, callback, error) {
                Log.log("添加会话");
                that.createAjaxFun(params, callback, "/session", 1, error, null, "core")
            },
            // 删除会话
            delChatList: function (params, callback, error) {
                Log.log("删除会话");
                var url = "/session/" + params.userId + '/' + params.sessionId;
                that.createAjaxFun({}, callback, url, 'delete', error, null, "core")
            },
        };
    ...
    var AntClient = function (option) {
         ...
         * @returns {*}
         */
        Interface.prototype.addChatList = function (params, successBack, failBack) {
            this.httpClientProxy.RoomBusy.addChatList(params, function(data){
                if (data && data.errorCode == 0) {
                    successBack && successBack(data.data);
                }else{
                    failBack && failBack(data);
         ...
         * @returns {*}
         */
        Interface.prototype.delChatList = function (params, successBack, failBack) {
            this.httpClientProxy.RoomBusy.delChatList(params, function(data){
                if (data && data.errorCode == 0) {
                    successBack && successBack(data.data);
                }else{
                    failBack && failBack(data);
        ...
        // 创建Session id
        Interface.prototype.createSessionId = createSessionId;
